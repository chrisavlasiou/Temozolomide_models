# Store health utility values
health_utilities <- list(
  stable = 0.743,
  progressive = 0.731,
  decrement_per_month = 0.02
)

# Store cost values
costs <- list(
  standard_TMz = list(
    stable_months_1_3 = 1944,
    stable_months_4_6 = 2453,
    stable_subsequent = 350,
    progressive = 2720,
    death = 0
  ),
  long_term_TMz = list(
    stable_months_1_3 = 1944,
    stable_months_4_6 = 2453,
    stable_subsequent = 2453,
    progressive = 2720,
    death = 0
  )
)

# Store model time parameters
model_params <- list(
  cycle_length = 1,  # in months
  time_horizon = 60  # in months
)

# Define OS and PFS values for Standard TMZ therapy
OS_standard <- c(0.988, 0.970, 0.950, 0.913, 0.888, 0.863, 0.813, 0.782, 0.733, 0.675, 
                 0.650, 0.611, 0.550, 0.525, 0.488, 0.450, 0.430, 0.394, 0.350, 0.325, 
                 0.313, 0.300, 0.275, 0.265, 0.240, 0.225, 0.213, 0.213, 0.213, 0.213, 
                 0.188, 0.188, 0.188, 0.188, 0.188, 0.188, 0.188, 0.188, 0.188, 0.188, 
                 0.094, 0.087, 0.081, 0.075, 0.070, 0.065, 0.060, 0.056, 0.052, 0.048, 
                 0.045, 0.041, 0.038, 0.036, 0.033, 0.030, 0.028, 0.026, 0.024, 0.022)

PFS_standard <- c(0.978, 0.968, 0.800, 0.738, 0.643, 0.539, 0.493, 0.459, 0.417, 0.368, 
                  0.314, 0.269, 0.259, 0.228, 0.227, 0.211, 0.197, 0.184, 0.174, 0.157, 
                  0.146, 0.136, 0.126, 0.107, 0.097, 0.097, 0.097, 0.093, 0.093, 0.093, 
                  0.071, 0.057, 0.033, 0.033, 0.033, 0.033, 0.024, 0.022, 0.020, 0.018, 
                  0.016, 0.014, 0.013, 0.011, 0.010, 0.009, 0.008, 0.007, 0.007, 0.006, 
                  0.005, 0.005, 0.004, 0.004, 0.003, 0.003, 0.003, 0.003, 0.002, 0.002)

# Calculate transition probabilities
transition_probs_standard <- data.frame(
  cycle = 1:60,
  prob_stable_to_death = 1 - exp(log(OS_standard) / (1:60)),
  prob_stable_to_progressive = 1 - exp(log(PFS_standard) / (1:60)),
  prob_progressive_to_death = 1 - exp(log(OS_standard - PFS_standard) / (1:60))
)

# Calculate stable to stable and progressive to progressive transition probabilities
transition_probs_standard$prob_stable_to_stable <- 1 - (transition_probs_standard$prob_stable_to_death + transition_probs_standard$prob_stable_to_progressive)
transition_probs_standard$prob_progressive_to_progressive <- 1 - transition_probs_standard$prob_progressive_to_death

# Define OS and PFS values for Long-term TMZ therapy
OS_long_term <- c(1.000, 1.000, 0.982, 0.930, 0.895, 0.860, 0.825, 0.789, 0.737, 0.702,
                  0.667, 0.649, 0.614, 0.579, 0.544, 0.526, 0.491, 0.456, 0.421, 0.404,
                  0.368, 0.351, 0.351, 0.351, 0.351, 0.333, 0.333, 0.316, 0.316, 0.296,
                  0.296, 0.296, 0.296, 0.296, 0.296, 0.269, 0.241, 0.215, 0.215, 0.215,
                  0.215, 0.215, 0.215, 0.215, 0.215, 0.215, 0.215, 0.215, 0.215, 0.215,
                  0.215, 0.215, 0.215, 0.215, 0.215, 0.215, 0.215, 0.215, 0.215, 0.215)

PFS_long_term <- c(0.947, 0.912, 0.877, 0.754, 0.699, 0.607, 0.497, 0.478, 0.421, 0.402,
                   0.364, 0.325, 0.306, 0.306, 0.306, 0.287, 0.268, 0.268, 0.268, 0.268,
                   0.268, 0.268, 0.268, 0.268, 0.268, 0.268, 0.268, 0.268, 0.268, 0.244,
                   0.244, 0.244, 0.244, 0.244, 0.244, 0.244, 0.203, 0.203, 0.203, 0.203,
                   0.203, 0.203, 0.203, 0.203, 0.203, 0.203, 0.203, 0.152, 0.152, 0.152,
                   0.152, 0.152, 0.152, 0.152, 0.152, 0.152, 0.152, 0.152, 0.152, 0.152)

# Calculate transition probabilities
transition_probs_long_term <- data.frame(
  cycle = 1:60,
  prob_stable_to_death = 1 - exp(log(OS_long_term) / (1:60)),
  prob_stable_to_progressive = 1 - exp(log(PFS_long_term) / (1:60)),
  prob_progressive_to_death = 1 - exp(log(OS_long_term - PFS_long_term) / (1:60))
)

# Calculate stable to stable and progressive to progressive transition probabilities
transition_probs_long_term$prob_stable_to_stable <- 1 - (transition_probs_long_term$prob_stable_to_death + transition_probs_long_term$prob_stable_to_progressive)
transition_probs_long_term$prob_progressive_to_progressive <- 1 - transition_probs_long_term$prob_progressive_to_death

# Function to calculate Markov trace
generate_markov_trace <- function(transition_probs) {
  cycles <- 60
  markov_trace <- data.frame(
    cycle = 1:cycles,
    stable = rep(0, cycles),
    progressive = rep(0, cycles),
    death = rep(0, cycles)
  )
  
  # Initial state distribution
  markov_trace$stable[1] <- 1  # Start with 100% in stable state
  markov_trace$progressive[1] <- 0
  markov_trace$death[1] <- 0
  
  # Iteratively apply transition probabilities
  for (t in 2:cycles) {
    if (t <= 3) {
      prev_t <- 1  # Use transition probabilities from cycle 1 for cycles 2 and 3
    } else {
      prev_t <- t - 2  # Use transition probabilities from two cycles earlier
    }
    
    markov_trace$stable[t] <- markov_trace$stable[t - 1] * transition_probs$prob_stable_to_stable[prev_t]
    markov_trace$progressive[t] <- (markov_trace$stable[t - 1] * transition_probs$prob_stable_to_progressive[prev_t]) +
      (markov_trace$progressive[t - 1] * transition_probs$prob_progressive_to_progressive[prev_t])
    markov_trace$death[t] <- 1 - (markov_trace$stable[t] + markov_trace$progressive[t])
  }
  
  return(markov_trace)
}

# Generate Markov traces for both treatments
markov_trace_standard <- generate_markov_trace(transition_probs_standard)
markov_trace_long_term <- generate_markov_trace(transition_probs_long_term)

# Function to calculate QALMs for each cycle
calculate_qalms <- function(markov_trace, health_utilities) {
  cycles <- nrow(markov_trace)
  qalms <- numeric(cycles)
  
  # Initialize progressive utility
  progressive_utilities <- rep(health_utilities$progressive, cycles)
  
  # Apply decrement for progressive state from cycle 3 to cycle 25
  for (t in 3:25) {
    progressive_utilities[t] <- max(health_utilities$progressive - (t - 2) * health_utilities$decrement_per_month, 0)
  }
  
  # Fix the progressive utility after cycle 25
  progressive_utilities[26:cycles] <- progressive_utilities[25]
  
  # Calculate QALMs for each cycle
  for (t in 1:cycles) {
    qalms[t] <- (markov_trace$stable[t] * health_utilities$stable) + 
      (markov_trace$progressive[t] * progressive_utilities[t])
  }
  
  # Compute total QALMs
  total_qalms <- sum(qalms)
  
  return(list(qalms = qalms, total_qalms = total_qalms))
}

# Calculate QALMs for both treatments
qalms_standard <- calculate_qalms(markov_trace_standard, health_utilities)
qalms_long_term <- calculate_qalms(markov_trace_long_term, health_utilities)

# Calculate total QALYs
total_qalys_standard <- qalms_standard$total_qalms / 12
total_qalys_long_term <- qalms_long_term$total_qalms / 12

# Print results
total_qalys_standard
total_qalys_long_term

# Function to calculate costs for each cycle
calculate_costs <- function(markov_trace, costs) {
  cycles <- nrow(markov_trace)
  costs_per_cycle <- numeric(cycles)
  
  # Define cost per cycle periods
  cost_stable <- c(rep(costs$stable_months_1_3, 4),  # Apply months 1-3 cost for cycles 1-4
                   rep(costs$stable_months_4_6, 3),  # Apply months 4-6 cost for cycles 5-7
                   rep(costs$stable_subsequent, 53))  # Apply subsequent months cost for cycles 8-60
  
  # Calculate costs for each cycle
  for (t in 1:cycles) {
    costs_per_cycle[t] <- (markov_trace$stable[t] * cost_stable[t]) +
      (markov_trace$progressive[t] * costs$progressive) +
      (markov_trace$death[t] * costs$death)
  }
  
  # Compute total costs
  total_costs <- sum(costs_per_cycle)
  
  return(list(costs_per_cycle = costs_per_cycle, total_costs = total_costs))
}

# Calculate costs for both treatments
costs_standard <- calculate_costs(markov_trace_standard, costs$standard_TMz)
costs_long_term <- calculate_costs(markov_trace_long_term, costs$long_term_TMz)

# Print total costs
total_costs_standard <- costs_standard$total_costs
total_costs_long_term <- costs_long_term$total_costs

total_costs_standard
total_costs_long_term

# Function to calculate ICER
calculate_icer <- function(costs_treatment_1, qalms_treatment_1, costs_treatment_2, qalms_treatment_2) {
  incremental_cost <- costs_treatment_2 - costs_treatment_1
  incremental_qalms <- qalms_treatment_2 - qalms_treatment_1
  incremental_qalys <- incremental_qalms / 12
  
  icer_qalms <- round(incremental_cost / incremental_qalms, 0)
  icer_qalys <- round(incremental_cost / incremental_qalys, 0)
  
  return(list(icer_qalms = icer_qalms, icer_qalys = icer_qalys))
}

# Calculate ICER for QALMs and QALYs
icer_results <- calculate_icer(
  costs_treatment_1 = total_costs_standard,
  qalms_treatment_1 = qalms_standard$total_qalms,
  costs_treatment_2 = total_costs_long_term,
  qalms_treatment_2 = qalms_long_term$total_qalms
)

# Print ICER results
icer_results$icer_qalms
icer_results$icer_qalys
